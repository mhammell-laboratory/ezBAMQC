...
def ovp_gene(geneIdx,exon_blocks1,exon_blocks2,strand) :
    
        (genes1,cds1,utr5_1,utr3_1,intron1,itgUp1k_1,itgDn1k_1) = geneIdx.Gene_annotation(exon_blocks1,strand)
        (genes2,cds2,utr5_2,utr3_2,intron2,itgUp1k_2,itgDn1k_2) = geneIdx.Gene_annotation(exon_blocks2,strand)
        ...
        if len(genes1) > 0 and len(genes2) > 0 :
            ...
            if len(genes) == 1 :
                gene = genes[0]
        if len(genes1) ==1  and len(genes2) == 0 :
            gene = genes1[0]
        if len(genes2) == 1 and len(genes1) == 0 :
            gene = genes2[0]
        ...
        if utr3_1 or utr3_2 :
            utr3_1 = True
        if intron1 or intron2 :
            intron1 = True
        if itgUp1k_1 or itgUp1k_2 :
            itgUp1k_1 = True
        if itgDn1k_1 or itgDn1k_2 :
            itgDn1k_1 = True
...
#    geneIdx = args[2]
    ...
    def qc (self,rRNAidx,geneIdx,mapq,outfile_fig,outfile) :
        ...
        try:
            while(1):
                ...
                for (cur_read1,cur_read2) in cur_reads :
                    ...
                    #read duplicate rate
                    rDup_prof.count(RNA_read1,RNA_read2,exons1,exons2,strand1,strand2)
                    #rRNA contamination
                    if rRNAidx is not None :
                        if rRNAidx.is_rRNA(exons1,strand1) or rRNAidx.is_rRNA(exons2,strand1):
                            rRNA_read += 1
                            continue
                    #print("check gene ovp\n")
                    (gene,cds,utr5,utr3,intron,itgUp1k,itgDn1k) = ovp_gene(geneIdx,exons1,exons2,strand1)
                    #print(gene+"\n")
                    ...
                    if cur_read1 is not None :
                        ...
                        intron_blocks1 = fetch_intron(cur_read1.pos,cur_read1.cigar,self.format)
                        intron_blocks2 = []
                        if cur_read2 is not None :
                            intron_blocks2 = fetch_intron(cur_read2.pos,cur_read2.cigar,self.format)
        ...
        if res.is_pairEnd :
            clip_prof.write(res.total_reads*2)
        else :
        ...
        try :
            ...
            if total_exons != 0 :
                ...
                print >> ROUT, 'pie(c(' + str(total_exons-zero_exons) + ',' + str(zero_exons) + '),labels=c("Covered","Uncovered"),main="Exons",radius=0.6,clockwise=T,col=c("#AFF8DB","white"))\n'
                print >> ROUT, "dev.state = dev.off()"
            ROUT.close()
        except :
