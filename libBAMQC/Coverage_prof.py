...
class CoverageBody_prof:
    def __init__(self,outfile_data,outfile_fig,geneIdx):
        ...
        self.ranges = {}
        self.frag_num = 0
        self.geneCounts_list = [0] * len(geneIdx.features)
        self.gene_percentile_base = {}
        

        self.geneToidx = dict(zip(geneIdx.features,range(0,len(geneIdx.features))))
        
        for g in geneIdx.plus_gene_exons :
            gene_all_base = []
            for st_end_str in geneIdx.plus_gene_exons[g] :
                (st,end) = st_end_str.split(':')
        ...
        for g in geneIdx.minus_gene_exons :
            gene_all_base = []
            for st_end_str in geneIdx.minus_gene_exons[g] :
                (st,end) = st_end_str.split(':')
                gene_all_base.extend(range(int(st),int(end)))
    ...
    def write(self,totalReads):
        ...
        try :
            ...
            print >>OUT4, "max_x = round(log(max(knots(Fn)),10),0)"
            print >>OUT4, "xx = c(0,10^seq(0,max_x,by=1))"
            print >>OUT4, "y=Fn(xx)"
            print >>OUT4, 'plot(x=xx,y=y,type="b",col="blue",pch=20,xlab="Number of Reads",ylab="CDF of Genes")\n'
...
#update gene percentile counts
    def __per_base_count(self,gene,exon_blocks1,exon_blocks2) :
        ...
        exons1_sorted = sorted(exon_blocks1,key=lambda tup:tup[1])
        for (chr,st,end) in exons1_sorted :
            ovp_base.extend(range(st,end))
        
        exons2_sorted = sorted(exon_blocks2,key=lambda tup:tup[1])
        for (chr,st,end) in exons2_sorted :
            ovp_base.extend(range(st,end))
