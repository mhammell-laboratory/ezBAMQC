...
class InnerDist_prof:
    ...
    def write(self):
        ...
        print >>RS, 'hist(fragsize,probability=T,breaks=%d,xlab="Inner distance (bp)",main=paste(c("Median=",frag_median,";","Mean=",frag_mean,";","SD=",frag_sd),collapse=""),border="blue")' % len(self.window_left_bound)
        print >>RS, "lines(density(fragsize,bw=%d),col='red')" % (2*self.step)
        print >>RS, "abline(h=0,v=frag_median,lty=2)"# % (2*self.step)
