...
class ParseBED:
		...
		for line in self.f:
			...
			if(strand == '-'):
				...
				for st,end in zip(exon_start,exon_end):
					...
					#exonNum -= 1
					ret_lst.append([chrom,st,end,geneName])
			else:
				exonNum=1
				for st,end in zip(exon_start,exon_end):
					#FO.write(chrom + "\t" + str(st) + "\t" + str(end) + "\t" + geneName + "_exon_" + str(exonNum) + "\t"+ str(score) +"\t" + strand + '\n')
	...
	def getBedinfor(self,outfile=None):
		...
		intron_sizes=[]
		FO.write("geneID\t" + "Position\t" + "Exon_Num\t" + "Exon_Len_Sum\t" + "Exon_Len_Min\t" + "Exon_Len_Max\t" + "Exon_Len_Avg\t")
		FO.write("Intron_Len_Sum\t" + "Intron_Len_Min\t" + "Intron_Len_Max\t" + "Intron_Len_Avg\n")
		for line in self.f:
			intron_sizes=[]
			try:
			...
			FO.write(geneName + '\t'+chrom +':'+str(tx_start+1)+'-'+str(tx_end)+strand+'\t')
